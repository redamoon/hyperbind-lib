...
/**
 * `keyComboFromEvent` が受け取れるイベント形状
 *
 * `KeyboardEvent` と React の `KeyboardEvent` の双方を受け取れるように
 * 必要なプロパティだけを定義しています。
 */
...
/**
 * 修飾キー単体の押下として無視するキー名（正規化後）
 */
const BARE_MODIFIER_KEYS = ["capslock", "altgraph", "fn", "os", "unidentified"];

/**
 * キーボードイベントから正規化済みのキーの組み合わせを求めます
 *
 * KeyRecorder のように「押されたキーを記録する」用途で使用します。
 * 修飾キー単体（Shift だけを押した場合など）では空文字列を返します。
 *
 * @param event - キーボードイベント（React の合成イベントも可）
 * @returns 正規化されたキーの組み合わせ（例: `"ctrl+shift+s"`）、記録対象外の場合は空文字列
 *
 * @example
 * ```typescript
 * keyComboFromEvent({ key: "!", code: "Digit1", shiftKey: true, ... }); // "shift+1"
 * keyComboFromEvent({ key: "Shift", shiftKey: true, ... });             // ""
 * ```
 */
export function keyComboFromEvent(event: KeyComboEventLike): string {
  ...
  // 修飾キー単体の押下は組み合わせとして扱わない
  if ((MODIFIER_ORDER as readonly string[]).includes(key) || BARE_MODIFIER_KEYS.includes(key)) {
  ...
  if (event.shiftKey && SHIFT_SYMBOL_TO_DIGIT[event.key]) {
    key = SHIFT_SYMBOL_TO_DIGIT[event.key];
  }

  if (!key) return "";

  return normalizeKeyCombo([...modifiers, key].join("+"));
}
