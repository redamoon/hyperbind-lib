...
```bash
npm install @hyperbind-lib/react @hyperbind-lib/core
```

**注意:** `@hyperbind-lib/react` は `@hyperbind-lib/core` に依存しています。両方のパッケージをインストールしてください。
...
会計処理などで使用されるプリセットキーバインドを使用できます。
...
ブラウザや一般的なアプリケーションで使用されるキーバインド（Ctrl+S、F5、Ctrl+R など）を予約キーとして定義しています。

```tsx
import { isReservedKey, getReservedKeyWarning } from "@hyperbind-lib/react";
...
- `preventDefault`: デフォルトのブラウザ動作を防ぐか（デフォルト: `true`）
- その他の標準的な `input` 要素のプロパティ（`placeholder`, `value`, `onChange` など）
...
**戻り値:** 予約キーの場合は警告メッセージ、そうでない場合は`null`
