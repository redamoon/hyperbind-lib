...
- **@hyperbind-lib/core**: キーバインド管理のコアライブラリ
- **@hyperbind-lib/react**: React 用フックとコンポーネント
...
```bash
npm install @hyperbind-lib/core @hyperbind-lib/react
```
...
`@hyperbind-lib/react` では、ブラウザや一般的なアプリケーションで使用されるキーバインド（Ctrl+S、F5、Ctrl+R など）を予約キーとして定義しています。
